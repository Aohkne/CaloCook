...
function History() {
  ...
  const groupHistoryByDate = (history) => {
    ...
    return sortedDates.map((date) => ({
      date,
      items: grouped[date].sort((a, b) => new Date(b.consumedAt || b.createdAt) - new Date(a.consumedAt || a.createdAt))
    }));
  };
              ...
              {groupedHistory.map((dateGroup) => (
                <div key={dateGroup.date} className={cx('date-section')}>
                  ...
                  <div className={cx('date-summary')}>
                    <span>Total: {dateGroup.items.reduce((sum, item) => sum + (item.dish?.calorie || 0), 0)} kcal</span>
                  </div>
                </div>
              ))}
            </div>
