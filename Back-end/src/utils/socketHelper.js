class SocketHelper {
  ...
  chat = {
    ...
    emitNewMessage: (messageData, receiverId = null) => {
      ...
      this.io.to(`user_${messageData.senderId}`).emit('message_sent', messageData)
    },
    ...
    emitMessageDelete: (deleteData, conversationId = null, senderId = null) => {
      ...
      if (conversationId) {
        this.io.to(`conversation_${conversationId}`).emit('message_deleted', deleteData)
      }
    ...
    emitMessagesSeen: (seenData, conversationId = null, userId = null) => {
      ...
      // Emit to conversation participants
      if (conversationId) {
        this.io.to(`conversation_${conversationId}`).emit('messages_seen', seenData)
      }
